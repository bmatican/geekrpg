...
<div style="text-align:right;margin-bottom:5px;padding:2px;border-bottom:1px solid #ccc;">
  <a href="<?php echo Geek::path('solution/add/'.$problemID); ?>">Post a problem</a>
</div>
<?php
  
  define( 'MAX_BODY_SIZE', 200 );  
  
  if( count( $this->controller->solutions ) == 0 ){
    echo 'No problems found';
  } else {
    foreach( $this->controller->solutions as $k => $v ){
      ...
      $href = Geek::path("solution/view/$problemID/".$v['id']);
      echo <<<POST
        <div class="post">
